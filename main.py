...
shell_key_pattern = re.compile(r'https?://postbox\.com/([A-Za-z0-9_-]+)')
...
async def send_msg(client_type, client, to_user_id, text):
    if client_type == 'man':
        try:
            # 尝试将 user_id 解析成可用的 InputPeer 实体
            to_user_entity = await client.get_input_entity(to_user_id)
            await client.send_message(to_user_entity, text)
        except Exception as e:
            print(f"获取用户实体失败: {e}")
            await client.send_message('me', f"无法获取用户实体: {to_user_id}")
    else:
        await client.send_message(to_user_id, text)
    return
...
async def handle_wish_logic(bot_client, from_user_id: int, text: str, client_type, reply_to_message):
    match = shell_key_pattern.search(text)
    if not match:
        # 没有邮局连结
        await send_msg(client_type, bot_client, from_user_id, "要许愿请直接点连结")
        return
    
    shell_key = match.group(1)  # 提取 shell_key
    
    # 检查 user_point
    try:
        safe_execute("SELECT point FROM user_point WHERE user_id = %s", (from_user_id,))
    except Exception as e:
        print(f"查 user_point 出错: {e}")
        await send_msg(client_type, bot_client, from_user_id, "查询功德点时出错，请稍后重试。")
    
        return
    
    row = cursor.fetchone()
    if not row:
        await send_msg(client_type, bot_client, from_user_id, "你目前还没有功德点，请先满足别人的愿望。")
      
        return
    
    point = row[0]
    if point <= 0:
        await send_msg(client_type, bot_client, from_user_id, "你的功德点不足，要先满足别人的愿望。")
    ...
    try:
        safe_execute(
            "INSERT INTO bottle_wish (user_id, shell_key, fulfill_state) VALUES (%s, %s, 'pending')",
            (from_user_id, shell_key)
        )
        safe_execute(
            "UPDATE user_point SET point = point - 1 WHERE user_id = %s AND point > 0",
            (from_user_id,)
        )
        safe_execute(
            "INSERT INTO wish_log (user_id, log_type, point_delta) VALUES (%s, 'wish', -1)",
            (from_user_id,)
        )
        text = (f"✅ 许愿成功！10分钟内若有反馈会通知，否则会转入『许愿池』。\nShell Key: {shell_key}")
        await send_msg(client_type, bot_client, from_user_id, text)
        
    except Exception as e:
        print(f"插入 bottle_wish 出错: {e}")
        await send_msg(client_type, bot_client, from_user_id, "提交许愿时出错，请稍后再试。")
...
async def handle_user_private_text(event):
    ...
    elif shell_key_pattern.search(text):
        await handle_wish_logic(user_client, to_user_id, text, 'man', msg.id)
        return
...
async def aiogram_handle_private_text(message: types.Message):
    ...
    if file_unique_id_pattern.fullmatch(text):
        ...
        await handle_wish_logic(bot_client, to_user_id, text,  'bot', reply_to_message)
    else:
      
        await message.delete()

# —— Aiogram：Bot 私聊 媒体 处理 —— 
# 私聊媒体（图片/文档/视频）
